...
void initDynamixelTxPacket(DynamixelTxPacket *packet, uint8_t id, uint8_t instruction, uint8_t *params, uint16_t param_len) {
    ...
    packet->header[2] = 0xFD;
    packet->reserved = 0x00;
    packet->id = id;
    packet->instruction = instruction;
    uint16_t length = param_len + 3;
    packet->lengthL = length & 0x00FF;
    packet->lengthH = (length>>8) & 0x00FF;
    memcpy(packet->parameters, params, param_len);
    packet->crc = update_crc(0, (unsigned char*)packet, 5 + length);
}

// 다이나믹셀에 패킷 전송
void sendDynamixelPacket(UART_HandleTypeDef *huart, DynamixelTxPacket *packet, uint16_t param_len) {
    uint16_t total_length = 7 + param_len + 3; // Header(3) + Reserved(1) + ID(1) + Length(2) + CRC(2)

    uint8_t tx_buffer[255] = {0};

    memcpy(tx_buffer, packet->header, 5);
    tx_buffer[5] = packet->lengthL;
    tx_buffer[6] = packet->lengthH;
    tx_buffer[7] = (packet->instruction);      // instruction
    memcpy(tx_buffer + 8, packet->parameters, param_len); // Parameters
    tx_buffer[total_length - 2] = packet->crc & 0xFF; // CRC_L
    tx_buffer[total_length - 1] = (packet->crc >> 8) & 0xFF; // CRC_H

    HAL_UART_Transmit(huart, tx_buffer, total_length, 1000);
}
...
void sendDynamixel(UART_HandleTypeDef *huart, DynamixelTxPacket packet, uint8_t id, uint8_t instruction, uint8_t *params, uint16_t param_len) {
    initDynamixelTxPacket(&packet, id, instruction, params, param_len);
    sendDynamixelPacket(huart, &packet, param_len);
}
...
void write_instruction(UART_HandleTypeDef *huart, DynamixelTxPacket packet, uint8_t id, uint8_t RAM_ADD, int32_t value, uint8_t param_len) {
    ...
    // 주소 할당 (2바이트)
    params[0] = (RAM_ADD & 0xFF);
    params[1] = (RAM_ADD >> 8) & 0xFF;
    // value 값을 param_len에 따라 할당
    for(int i = 2; i < param_len; i++) {
        params[i] = (value >> (8 * (i-2))) & 0xFF;
    }

    sendDynamixel(huart, packet, id, DXL_INST_WRITE, params, param_len);
}
